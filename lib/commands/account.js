...
module.exports = function (imports) {
  ...
  account.task('redeem')
    ...
    .handler(function (code, done) {
      if (!code) return done(imports.errors.INVALID_VOUCHER);
      ...
      redeemVoucher().then(function (res) {
        ...
        if (res.statusCode == 304) return done(imports.errors.VOUCHER_USED_BY_YOU);
        ...
        if (res.statusCode == 404) return done(imports.errors.INVALID_VOUCHER);
        if (res.statusCode == 403) return done(imports.errors.VOUCHER_USED);
